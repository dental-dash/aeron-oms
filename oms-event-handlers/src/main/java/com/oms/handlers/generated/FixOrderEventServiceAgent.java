 ...
 */
@EventHandler
public class FixOrderEventServiceAgent implements CommandStream, Agent {

    private static final Log log = LogFactory.getLog(FixOrderEventServiceAgent.class);
    ...
    private FixOrderEventService fixOrderEventHandler;

    private final Class<?>[] classKeys = new Class<?>[6];
    private final Supplier<?>[] encoderFactories = new Supplier<?>[6];

    public FixOrderEventServiceAgent(Subscription sequencedEventSub, Publication ingressCommandPub) {
    ...
    public void setFixOrderEventService(FixOrderEventService eventHandler) {
